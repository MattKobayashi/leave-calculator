...
class LeavePeriod(BaseModel):
    ...
    is_range: bool = Field(default=False, description="Whether this is a date range or single day")


class LeaveCalculationRequest(BaseModel):
    start_date: str = Field(..., description="Employment start date or calculation period start")
    end_date: str = Field(..., description="Current date or calculation period end")
    annual_entitlement: float = Field(..., gt=0, description="Annual leave entitlement in days")
    leave_periods: List[LeavePeriod] = Field(default=[], description="List of leave periods taken")
        ...
        for period in leave_periods:
            try:
                ...
                if start_date > end_date:
                    raise ValueError(f"Invalid date range: {period.start_date} to {period.end_date}")
                
                days = (end_date - start_date).days + 1
                total_days += days
                
            except Exception as e:
                raise ValueError(f"Error processing leave period {period.start_date} to {period.end_date}: {str(e)}")
...
def validate_leave_periods(leave_periods: List[LeavePeriod]) -> None:
    ...
    for i, period in enumerate(leave_periods):
        try:
            ...
            if start_date > end_date:
                raise ValueError(f"Leave period {i+1}: Start date must be before or equal to end date")
...
async def calculate_leave(request: LeaveCalculationRequest):
    ...
    try:
        ...
        logger.info(f"Leave calculation completed: {result['result']['formatted']} with {len(request.leave_periods)} leave periods")
